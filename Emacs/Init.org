...
(use-package dired-single
  ...
  (if (boundp 'dired-mode-map)
      ...
      (my/dired-single-init)
    ;; it's not loaded yet, so add our bindings to the load-hook
    (add-hook 'dired-load-hook 'my/dired-single-init)))
...
(my/view-def
  ...
  "dd" '(dired-jump :which-here "Here")
  "dc" '(dired :which-key "Counsel")
...
(use-package lsp-ui
  ...
  ;; (lsp-ui-doc-show) ;; Gives error, must investigate
  )
...
(use-package org-journal
  ...
  (org-journal-search-forward-fn 'search-forward-regexp)
...
(use-package epa-file
  ...
  :after org
  :config
  ;; Default Mail of gpg key to encrypt Every key has assigned an Email. If a
  ;; key exists with `in-user-email'. Use it as default.
  (add-to-list 'epa-file-encrypt-to in-user-mail)
  :custom
